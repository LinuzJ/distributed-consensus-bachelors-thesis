...
\subsection{Population protocol}
 ...
 \begin{itemize}
     ...
     \fda{by reading this, I got convinced that it is better to keep state as "finite state machine", and "opinion" for the distributed consensus definition. The problem is that the set $Q$ of possible states is not referring to the opinions. Better said, some states might be opinions but others don't have to. I think the set $Y$ represents the final possible opinions, and $\Sigma$ the initial possible opinions.
     Maybe it is better to say opinion in the definition of distributed consensus, and then keep state as it is (and specify whether it is also an opinion like in the case of Angluin for the 2 states that are not the undecided one).}
 \end{itemize}
